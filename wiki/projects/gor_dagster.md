...
**Production RSS catalogue (thirteen `podcast_rss` ContentSources; SI via `get_podcast_rss_content_source_ids()`):** Fast Money (`49400b5b-6e3e-4c0d-be0b-8cd7ab18ba74`), Mad Money (`e6a22166-82ca-482d-b54b-4a1f016948c3`), Hedgeye (`8a61349e-9df2-4397-bc76-b4af8b0fb9d8`), Halftime Report (`c925324b-63e2-4f10-a074-6e4ed7da2d0b`), Morning Filter (`4f158ea6-c1c4-43f1-86e3-3d96fed6dd80` — [morning-filter-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/morning-filter-ingestion.md)), **Compound and Friends** (`0b75ea6c-20b2-4a7c-89db-b0899788a8cc` — [compound-and-friends-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/compound-and-friends-ingestion.md)), **7investing** (`c2658090-942b-4cbd-9552-f04995220873` — [7investing-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/7investing-ingestion.md), Anchor/Spotify feed `https://anchor.fm/s/1659b6fc/podcast/rss`, `external_id` `1659b6fc`, enclosure **Pattern 4** Megaphone `APO…` and **Pattern 5** Anchor `anchor.fm/s/...`), **The Acquirers Podcast** (`428270b1-e53b-494a-b1b5-4278756c6aa4` — [acquirers-podcast-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/acquirers-podcast-ingestion.md), Anchor/Spotify feed `https://anchor.fm/s/9603714/podcast/rss`, `external_id` `9603714`, Patterns 4+5, mid-catalog `.m4a` supported), **Motley Fool Hidden Gems Investing** (`22e0168c-ef5b-4864-a2ed-cbe8aff56d15` — [hidden-gems-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/hidden-gems-ingestion.md), Megaphone direct RSS `feeds.megaphone.fm/ARML8165884693`, `external_id` `ARML8165884693`, enclosure **Pattern 6** Megaphone `ARML…`), **Investing Unscripted** (`ff6a3fbe-590d-4693-88f3-d8b17be98700` — [investing-unscripted-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/investing-unscripted-ingestion.md), Megaphone direct RSS `feeds.megaphone.fm/ARML8770731717`, `external_id` `ARML8770731717`, Pattern 6 twin, **272/272** downloaded **2026-07-27**; gor-blog directory Covered `investing-unscripted`), and **Chit Chat Stocks** (`ca015224-3d4c-44cf-9428-437a52516988` — [chit-chat-stocks-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/chit-chat-stocks-ingestion.md), Megaphone direct RSS `feeds.megaphone.fm/ARML5007448648`, `external_id` `ARML5007448648`, Pattern 6 twin via iTunes feed discovery, **801/802** downloaded **2026-08-02**; directory Covered mapping deferred), and **The Intrinsic Value Podcast** (`5fdb4382-43bb-414d-9378-11827090e223` — [intrinsic-value-podcast-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/intrinsic-value-podcast-ingestion.md), Megaphone direct RSS `feeds.megaphone.fm/PPLLC9062165992`, `external_id` `PPLLC9062165992`, enclosure **Pattern 7** Megaphone `PPLLC…`, **784/784** downloaded **2026-08-19**; gor-blog directory Covered `the-intrinsic-value-podcast`), and **Hedge Fund Tips with Tom Hayes** (`e707e222-d1e4-401b-bea3-4b489d835c27`, Anchor/Spotify feed `https://anchor.fm/s/12172a20/podcast/rss`, `external_id` `12172a20`, enclosure **Pattern 4** Megaphone `APO…` and **Pattern 5** Anchor, 359-episode preflight **2026-09-18**, **359/359** downloaded **2026-09-21**, newest-first, 0 duplicate groups). Episode ID extraction: Patterns 1–7 in [`rss_ingestion.py`](file:///Users/andreskull/gor_dagster/gor_dagster/assets/rss_ingestion.py). SI discovery: [`get_podcast_rss_content_source_ids`](file:///Users/andreskull/gor_dagster/gor_dagster/sensors/si_sensor.py) (all active `podcast_rss` sources — no hardcoded allowlist since **2026-07-01**).
...
└── marketing/google-ads/               ← PMax creative archive (write-once + live.json)
```
...
| Pattern 7 Megaphone PPLLC | Last enclosure branch for `PPLLC{digits}.mp3`; Megaphone slug `PPLLC…` as `external_id`. Shared TCP/APO/ARML/PPLLC matcher anchors prefix to the **final path segment**; `.mp3` only. [intrinsic-value-podcast-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/intrinsic-value-podcast-ingestion.md). |
| Show name vs slug freeze | Trim iTunes network suffixes on `ContentSource.name` **before** first `mat_shows` sync; slugs do not follow a later rename. `details_bq` overwrites name from RSS title. [intrinsic-value-podcast-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/intrinsic-value-podcast-ingestion.md). |
...
| Long-episode catalog hold | Hide a new `podcast_rss` source with `ContentSource.url` NULL until one proof episode passes STT, SI, and FE. ElevenLabs waits: signed URL 14400s, POST 3600s, poll 7200s (credit pacing unchanged). A merged monologue’s real end is `utterance_end`; an FE window can end after the last prediction. Directory card only after `/show/{slug}` returns 200. [hedge-fund-tips-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/hedge-fund-tips-ingestion.md) |
...
**Wrapped 2026-08-22:** `intrinsic-value-podcast-ingestion/` → [intrinsic-value-podcast-ingestion.md](file:///Users/andreskull/gor_dagster/docs/architecture/features/intrinsic-value-podcast-ingestion.md) (784/784 downloaded; Pattern 7 PPLLC; gor-blog Covered `the-intrinsic-value-podcast`; app `/show/the-intrinsic-value-podcast`; STT/SI/FE draining)
...
| Google Ads PMax creatives (monthly refresh) | [[concepts/google-ads-creative-assets]]; runbook [google-ads-asset-refresh.md](file:///Users/andreskull/gor_dagster/docs/runbooks/google-ads-asset-refresh.md); `scripts/build_google_ads_variants.py`, `scripts/archive_google_ads_run.py` |
